...
def report_configuration():
    ...
    """
    config = odoo.tools.config
    _logger.info("ADAX-ERP %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Usando o arquivo de configuração em " + config.rcfile)
    _logger.info('caminhos dos aplicativos: %s', odoo.modules.module.ad_paths)
    ...
    _logger.info('base de dados: %s@%s:%s', user, host, port)
