...
var ChatAction = Widget.extend(ControlPanelMixin, {
    ...
    _onInviteButtonClicked: function () {
        var title = _.str.sprintf(_t('Convide pessoas para #%s'), this.channel.name);
    ...
    _onRequestNotificationPermission: function (event) {
        ...
        if (def) {
            def.then(function (value) {
                if (value !== 'granted') {
                    utils.send_notification(self, _t('Permissão negada'),
                        _t('O sistema não terá permissão para enviar notificações nativas neste dispositivo.'));
                } else {
                    utils.send_notification(self, _t('Permissão consedida'),
                        _t('O sistema agora tem permissão para enviar notificações nativas para você neste dispositivo.'));
                }
            });
        }
    },
