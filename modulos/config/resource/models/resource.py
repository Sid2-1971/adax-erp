...
class ResourceCalendar(models.Model):
    ...
    def default_get(self, fields):
        ...
        if not res.get('name') and res.get('company_id'):
            res['name'] = _('Horas de trabalho of %s') % self.env['res.company'].browse(res['company_id']).name
        return res

    def _get_default_attendance_ids(self):
        return [
            (0, 0, {'name': _('Manhã'), 'dayofweek': '0', 'hour_from': 8, 'hour_to': 12}),
            (0, 0, {'name': _('Tarde'), 'dayofweek': '0', 'hour_from': 13, 'hour_to': 17}),
            (0, 0, {'name': _('Manhã'), 'dayofweek': '1', 'hour_from': 8, 'hour_to': 12}),
            (0, 0, {'name': _('Tarde'), 'dayofweek': '1', 'hour_from': 13, 'hour_to': 17}),
            (0, 0, {'name': _('Manhã'), 'dayofweek': '2', 'hour_from': 8, 'hour_to': 12}),
            (0, 0, {'name': _('Tarde'), 'dayofweek': '2', 'hour_from': 13, 'hour_to': 17}),
            (0, 0, {'name': _('Manhã'), 'dayofweek': '3', 'hour_from': 8, 'hour_to': 12}),
            (0, 0, {'name': _('Tarde'), 'dayofweek': '3', 'hour_from': 13, 'hour_to': 17}),
            (0, 0, {'name': _('Manhã'), 'dayofweek': '4', 'hour_from': 8, 'hour_to': 12}),
            (0, 0, {'name': _('Tarde'), 'dayofweek': '4', 'hour_from': 13, 'hour_to': 17})
        ]
...
class ResourceCalendarLeaves(models.Model):
    ...
    company_id = fields.Many2one(
        'res.company', related='calendar_id.company_id', string="Empresa",
        readonly=True, store=True)
    calendar_id = fields.Many2one('resource.calendar', 'Horas de trabalho')
