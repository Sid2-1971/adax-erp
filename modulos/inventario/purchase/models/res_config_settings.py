...
class ResConfigSettings(models.TransientModel):
    ...
    lock_confirmed_po = fields.Boolean("Bloquear pedidos confirmados", default=lambda self: self.env.user.company_id.po_lock == 'lock')
    po_lock = fields.Selection(related='company_id.po_lock', string="Purchase Order Modification *")
    po_order_approval = fields.Boolean("Ordem de aprovação", default=lambda self: self.env.user.company_id.po_double_validation == 'two_step')
    po_double_validation = fields.Selection(related='company_id.po_double_validation', string="Levels of Approvals *")
    po_double_validation_amount = fields.Monetary(related='company_id.po_double_validation_amount', string="Quantidade mínima", currency_field='company_currency_id')
    ...
    default_purchase_method = fields.Selection([
        ...
        ], string="Controle de faturamento", default_model="product.template",
    ...
    module_purchase_requisition = fields.Boolean("Contratos de compra")
