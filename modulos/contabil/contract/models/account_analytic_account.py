...
class AccountAnalyticAccount(models.Model):
    ...
    recurring_invoices = fields.Boolean(
        string='Gerar faturas recorrentes automaticamente',
    )
    recurring_next_date = fields.Date(
        default=fields.Date.context_today,
        copy=False,
        string='Data da próxima fatura',
    )
    ...
    def _onchange_contract_template_id(self):
        """Atualize os campos do contrato com o do modelo.

        Tome especial atenção com o `recurring_invoice_line_ids`,
        que deve ser criado usando os dados das linhas de contrato. Cascata
        A exclusão garante que todas as linhas erradas criadas também sejam
        excluído.
        """
        ...
        for contract in self.filtered('recurring_invoices'):
            if not contract.partner_id:
                raise ValidationError(
                    _("Você deve fornecer um cliente para o contrato '%s'") %
                    contract.name
                )
    ...
    def _check_recurring_next_date_start_date(self):
        for contract in self.filtered('recurring_next_date'):
            if contract.date_start > contract.recurring_next_date:
                raise ValidationError(
                    _("Você não pode ter a próxima data de faturamento antes do início "
                      "do contrato '%s'") % contract.name
                )
    ...
    def _check_recurring_next_date_recurring_invoices(self):
        for contract in self.filtered('recurring_invoices'):
            if not contract.recurring_next_date:
                raise ValidationError(
                    _("Você deve fornecer uma próxima data de faturamento para o contrato "
                      "'%s'") % contract.name
                )
    ...
    def _check_date_start_recurring_invoices(self):
        for contract in self.filtered('recurring_invoices'):
            if not contract.date_start:
                raise ValidationError(
                    _("Você deve fornecer uma data de início para o contrato '%s'") %
                    contract.name
                )
    ...
    def _check_start_end_dates(self):
        for contract in self.filtered('date_end'):
            if contract.date_start > contract.date_end:
                raise ValidationError(
                    _("A data de início do contrato '%s' não pode ser posterior à data de término")
                    % contract.name
                )
